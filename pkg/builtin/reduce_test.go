...
func TestBuiltin_Reduce__InPorts(t *testing.T) {
	...
	o, err := MakeOperator(core.InstanceDef{Operator: "slang.reduce", Generics: map[string]*core.TypeDef{"itemType": {Type: "number"}}})
	r.NoError(err)
	...
	o, err = MakeOperator(core.InstanceDef{Operator: "slang.reduce", Generics: map[string]*core.TypeDef{"itemType": {Type: "string"}}})
	r.NoError(err)
